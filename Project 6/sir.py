...
def has_an_infected_neighbor(city, location):
    ...
    if location == 0:
        return 'I' in city[-1] or 'I' in city[1]
    elif location == len(city)-1:
        return 'I' in city[-2] or 'I' in city[0]
    else:
        return 'I' in city[location-1] or 'I' in city[location+1]
    ...
    if disease_state == 'S':
        if has_an_infected_neighbor(city, location):
           return ('I',0)
            ...
            return ('S',d+1)
    elif disease_state == 'I':
        if d+1 < days_contagious:
           return ('I',d+1)
        else:
            return ('R',0) 
    elif disease_state == 'R':
        return ('R', d+1)
    else:
        return (disease_state, d+1)
...
def simulate_one_day(starting_city, days_contagious):
    ...
    return [advance_person_at_location(starting_city, i, days_contagious) for i in range(len(starting_city))]
...
def is_transmission_possible(city):
    ...
    for i in range(len(city)):
        disease_state = city[i][0]
        if disease_state == 'S' and has_an_infected_neighbor(city, i):
            return True
    return False
...
def run_simulation(starting_city, days_contagious):
    ...
    city = starting_city
    while True :
        if not is_transmission_possible(city):
            break
...
def vaccinate_person(vax_tuple):
    ...
    disease_state, days_of_the_state, eagerness_to_be_vaccinate = vax_tuple
    if disease_state == 'R' or disease_state ==  'I':
        return (disease_state, days_of_the_state)

    elif disease_state == 'S' :
    
        if random.random() < eagerness_to_be_vaccinate: 
            return ('V',0)
        else:
            return ('S',days_of_the_state)
...
def vaccinate_city(city_vax_tuples, random_seed):
    ...
    for i in range(len(city_vax_tuples)):
        new_city.append(vaccinate_person(city_vax_tuples[i]))
    return(new_city)
...
def vaccinate_and_simulate(city_vax_tuples, days_contagious, random_seed):
    ...
    return (run_simulation((vaccinate_city(city_vax_tuples, random_seed)), days_contagious))
